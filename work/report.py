...
def alphabet_case_section(analysis):
    ...
    w("")
    goog_s = next((c["summary"] for c in std if c["ticker"] == "GOOG"), {})
    w(f"알파벳은 함축수익률 자체로는 상위권이되 1위가 아닙니다. 눈에 띄는 건 **조합**입니다. "
      f"위에 있는 종목들은 대부분 품질이 낮거나 순부채 상태인데, 알파벳은 ROIC 중앙값 "
      f"{pct(goog_s.get('roic_10y_median'))}에 "
      f"{goog_s.get('roic_above_10pct_years')}년 내내 두 자릿수를 지켰고 목록에서 "
      f"가장 큰 순현금을 들고 있습니다. 품질·재무구조·함축수익률을 함께 놓고 보면 "
      f"메가캡 중에서 가장 나은 조합입니다.")
    w("")
...
def limitations_section(analysis, verification):
    ...
    w("")
    w("**ROIC의 분자는 보고된 영업이익입니다.** 초판은 전 종목을 `세전이익 + 이자비용`으로 "
      "통일했습니다. 목적은 태깅이 회사마다 다른 문제를 피하는 것이었는데, 대가가 더 컸습니다. "
      "이 식은 이자수익과 투자평가익까지 분자로 끌고 들어오는데, 분모인 투하자본은 현금을 "
      "빼고 있습니다. 현금이 벌어들인 것을 현금을 뺀 자본으로 나누는 셈이라 비율의 의미가 "
      "무너집니다. 실제 왜곡도 작지 않았습니다 — 알파벳 FY2025는 $159.6B 대 $129.0B로 24% "
      "차이였고, 인텔 FY2024는 +$2.6B와 −$2.2B로 부호가 뒤집혔으며, 34개사 중 10개사가 "
      "10%를 넘겼습니다. 그래서 보고된 영업이익을 우선으로 되돌리고, 영업이익 소계를 "
      "태깅하지 않는 연도만 `세전이익 + 이자비용`으로 대체했습니다. 어느 방식을 썼는지는 "
      "연도별로 `analysis.json`의 `ebit_method`에 남아 있습니다.")
    w("")
    w("**주주이익은 투자평가손익을 걷어낸 뒤에 계산합니다.** 2018년 회계기준 변경 이후 보유 "
      "지분의 평가손익이 손익계산서를 통과합니다. 현금도 아니고 반복되지도 않으며 사업이 번 "
      "돈도 아닌데, 규모는 알파벳 FY2025 세전 $24.6B, 버크셔 $39.1B, 엔비디아 $8.9B입니다. "
      "버핏이 최근 주주서한마다 한 문단씩 써서 '이 줄은 무시하고 영업이익을 보라'고 말하는 "
      "바로 그 항목이라, 세후로 차감한 뒤 주주이익을 산출했습니다. 태깅되지 않은 회사는 "
      "순이익을 그대로 쓰되 기타손익이 영업이익의 10%를 넘는 연도는 별도로 표시했습니다.")
    w("")
    w("**주식보상비용은 되더하지 않습니다.** 주주이익의 기본 경로(순이익 + 감가상각 − 설비투자 "
      "− 운전자본증감)에서는 이미 비용으로 빠져 있지만, 대차대조표가 유동/비유동으로 구분되지 "
      "않아 `영업현금흐름 − 설비투자`로 대체하는 42개 연도행에서는 영업현금흐름이 이를 "
      "비현금항목으로 되더해 놓습니다. 같은 회사가 어느 경로를 타느냐에 따라 다르게 평가되는 "
      "것을 막기 위해 대체 경로에서 주식보상비용을 다시 뺐습니다. 버핏의 1992년 서한이 "
      "근거입니다 — \"스톡옵션이 보상이 아니면 무엇인가? 보상이 비용이 아니면 무엇인가?\"")
    w("")
