...
YEAR_FORMULAS = [
    # 보고 영업이익이 먼저입니다. 투하자본에서 현금을 빼고 있으므로, 그 현금이 벌어들인
    # 이자수익과 투자평가익도 분자에서 빠져야 같은 자본에 대한 수익률이 됩니다.
    ("영업이익(EBIT)", 13, USD_B,
     '=IF(ISNUMBER({영업이익(보고)}),{영업이익(보고)},'
     'IF(AND(ISNUMBER({세전이익}),ISNUMBER({이자비용})),{세전이익}+{이자비용},'
     'IF(ISNUMBER({세전이익}),{세전이익},"")))'),
    ("실효세율", 10, PCT,
    ...
    # 주주이익의 출발점인 '보고 이익'에서 투자평가손익을 세후로 뺍니다. 회계기준상
    # 손익계산서를 통과할 뿐 현금도 아니고 반복되지도 않으며, 사업이 번 돈도 아닙니다.
    ("영업순이익", 13, USD_B,
     '=IF(AND(ISNUMBER({순이익}),ISNUMBER({투자평가손익}),ISNUMBER({실효세율})),'
     '{순이익}-{투자평가손익}*(1-{실효세율}),IF(ISNUMBER({순이익}),{순이익},""))'),
    ("주주이익", 13, USD_B,
     '=IF({업종구분}="금융","",'
     'IF(AND(ISNUMBER({영업순이익}),ISNUMBER({감가상각}),ISNUMBER({설비투자}),ISNUMBER({운전자본증감})),'
     '{영업순이익}+{감가상각}-{설비투자}-{운전자본증감},'
     'IF(AND(ISNUMBER({영업현금흐름}),ISNUMBER({설비투자})),'
     '{영업현금흐름}-{설비투자}-IF(ISNUMBER({주식보상비용}),{주식보상비용},0),"")))'),
    ("주주이익 산출방식", 30, None,
     '=IF({업종구분}="금융","금융업 - 정의되지 않음",'
     'IF(AND(ISNUMBER({영업순이익}),ISNUMBER({감가상각}),ISNUMBER({설비투자}),ISNUMBER({운전자본증감})),'
     '"영업순이익+감가상각-설비투자-운전자본증감",'
     'IF(AND(ISNUMBER({영업현금흐름}),ISNUMBER({설비투자})),'
     'IF(ISNUMBER({주식보상비용}),"영업현금흐름-설비투자-주식보상비용",'
     '"영업현금흐름-설비투자(주식보상비용 미태깅)"),"산출불가")))'),
    ("주주이익률", 11, PCT,
...
DEFINITIONS = [
    ...
    ("연도별계산", "영업이익(EBIT)", "보고된 영업이익 (없으면 세전이익 + 이자비용)",
     "보고된 영업이익을 먼저 씁니다. 투하자본이 현금을 빼고 있으므로, 그 현금과 투자자산이 벌어들인 "
     "이자수익·투자평가익도 분자에서 빠져야 같은 자본에 대한 수익률이 됩니다. '세전이익+이자비용'은 "
     "그것들을 그대로 끌고 들어옵니다 — 알파벳 FY2025는 그 차이가 $159.6B 대 $129.0B로 24%였고, "
     "인텔 FY2024는 부호까지 뒤집혔습니다(+$2.6B 대 −$2.2B). 릴리·머크·KLA처럼 영업이익 소계를 "
     "태깅하지 않는 연도만 '세전이익+이자비용'으로 대체하고, 그 사실을 산출방식 열에 남깁니다.",
     "같은 행의 원천 셀"),
    ...
    ("연도별계산", "영업순이익", "순이익 − 투자평가손익 × (1 − 실효세율)",
     "주주이익의 출발점인 '보고 이익'을 사업의 이익으로 되돌리는 조정입니다. 2018년 회계기준 "
     "변경(ASU 2016-01) 이후 보유 주식·채권의 평가손익이 손익계산서를 통과하는데, 현금도 아니고 "
     "반복되지도 않으며 사업이 번 돈도 아닙니다. 알파벳 FY2025 세전 $24.6B, 버크셔 $39.1B, "
     "엔비디아 $8.9B가 여기에 해당합니다. 버핏이 최근 주주서한마다 한 문단씩 할애해 '이 줄은 "
     "무시하고 영업이익을 보라'고 말하는 바로 그 항목입니다. 태깅되지 않은 회사는 순이익을 "
     "그대로 쓰되, 기타손익이 영업이익의 10%를 넘으면 그 사실을 별도로 표시합니다.", "같은 행"),
    ("연도별계산", "주주이익", "영업순이익 + 감가상각 − 설비투자 − 운전자본증감",
     "버핏의 owner earnings 정의입니다. 네 항목이 모두 같은 회계기간이어야 합니다. "
     "대차대조표가 유동자산·유동부채로 구분되지 않아 운전자본증감을 낼 수 없는 연도에는 "
     "'영업현금흐름 − 설비투자 − 주식보상비용'으로 대체합니다. 주식보상비용을 빼는 이유는 "
     "영업현금흐름이 그것을 비현금항목으로 되더해 놓기 때문입니다 — 순이익 경로에서는 이미 "
     "비용으로 차감된 항목이므로, 빼지 않으면 같은 회사가 어느 경로를 타느냐에 따라 다르게 "
     "평가됩니다. 버핏의 1992년 주주서한: \"스톡옵션이 보상이 아니면 무엇인가? 보상이 비용이 "
     "아니면 무엇인가? 비용이 이익 계산에 들어가지 않는다면 도대체 어디로 가야 하는가?\"",
     "같은 행"),
